...
def load_passwords():
    ...
    if file_path.exists():
        if file_path.stat().st_size > 0:  # Check if the file is not empty
            with file_path.open("rb") as file:
                try:
                    names, usernames, hashed_passwords = pickle.load(file)
                except EOFError:
                    # Return empty lists if the file is corrupted or empty
...
def main():
    st.markdown("<h1 style='text-align: center;'>Xtract Login/Signup</h1>", unsafe_allow_html=True)
